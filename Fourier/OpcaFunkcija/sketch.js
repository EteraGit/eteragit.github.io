...
function setup()
{
    ...
    a_n.push(integrate_f(f, -Math.PI, Math.PI, 100) / 2);
    b_n.push(0);

    for(k = 1; k <= 200; k++)
    {
        a_n.push(integrate_f_trig(f_cos, f, k, -Math.PI, Math.PI, 100));
        b_n.push(integrate_f_trig(f_sin, f, k, -Math.PI, Math.PI, 100));
    }
...
function draw()
{
    ...
    let radius = scalar * a_n[0];
    x += radius * cos(time);
    y += radius * sin(time);

    stroke(50);
    noFill();
    ellipse(0, 0, Math.sqrt(x * x + y * y) * 2); 
    ...
    for(let i = 1; i < slider1.value() * 2; i++)
    {
        let prevx = x;
        let prevy = y;
