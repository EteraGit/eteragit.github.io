...
function setup()
{
    ...
    a_n.push(integrate_f(f, -Math.PI, Math.PI, 1000) / 2);
    b_n.push(0);

    for(k = 1; k <= 200; k++)
    {
        a_n.push(integrate_f_trig(f_cos, f, k, -Math.PI, Math.PI, 1000));
...
function draw()
{
    ...
    if (time >= 0 && time <= Math.PI) funkcija.unshift(scalar);
    else funkcija.unshift(-scalar);
